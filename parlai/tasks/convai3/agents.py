...
--task convai3:{TEACHER_NAME}:no_cands
...
class NormalizedTeacherTrait(object):
    ...
    def setup_data(self, path):
        logging.info(f"loading normalized convai3 data: {path}")
        exs_counter = 0

        dataset_directory = "parlai/tasks/convai3/huggingface"
        # check if dataset already downloaded in dataset_directory
        if not os.path.exists(dataset_directory):
            os.makedirs(dataset_directory)
        train_dataset_path = os.path.join(dataset_directory, "convai3_train.txt")
        validation_dataset_path = os.path.join(dataset_directory, "convai3_train.txt")
        if not os.path.exists(train_dataset_path):
            dataset = load_dataset("conv_ai_3")
            train_dataset, validation_dataset = dataset["train"], dataset["validation"]
            train_dataset_list = get_list_from_hfdataset(train_dataset),
            validation_dataset_list = get_list_from_hfdataset(validation_dataset)
            save_dataset(train_dataset_path, train_dataset_list)
            save_dataset(validation_dataset_path, validation_dataset_list)
        ...
        for data, new_episode in super().setup_data(train_dataset_path):
